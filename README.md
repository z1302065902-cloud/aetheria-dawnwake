...
> 当前版本：**Phase 1 里程碑（可玩垂直切片）+ 四轮硬化，含战斗表现升级**
...
> 响应式布局 **24/24**（6 视口 + 4 菜单页）· 泄漏压测 **6/6**。
>
> ⚠️ **第五轮「单人向重做」正在进行中（未验收完成）**：自动化（工人配比 / 自动排产）、编组与姿态、冒险层（宝箱/NPC/秘境/裂隙）、
> 每局随机祝福、十关全部解锁 + 程序化布点、地面去方块感、四足单位分层美术。
> 现状：`npm run typecheck` 通过；新增 `tests/campaign.mjs` **5/36 失败**（m01/m04/m06/m08/m09 走不到 Victory）。
> **接手先读 [`docs/PROGRESS.md`](docs/PROGRESS.md) 的「一之二。」**，那里有失败原文与下一步判据。
>
...
- **战役 02–10 与另两张地图：已解锁但未跑通**（第五轮 WIP）：10 关都能开局（地图连通/基地/营地/资源/中立怪/Boss 接线 36 项断言中 31 项通过），
