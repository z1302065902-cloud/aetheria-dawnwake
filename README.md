...
> 当前版本：**Phase 1 里程碑（可玩垂直切片）+ 三轮硬化**
> 一局完整可玩：进入游戏 → 选英雄 → 采集 → 建造 → 生产 → 带迷雾探索 → 战斗 → 英雄升级/阵亡复活 → 摧毁敌方营地 → Boss 现身 → 击败 Boss → 掉落装备 → 结算存档。
> 主线 15–30 分钟。
>
> **验收**：玩法端到端 **60/60**（dev 与生产构建各一遍）· 真实 GPU **8/8** · 响应式布局 **24/24**（6 视口 + 4 菜单页）· 泄漏压测 **6/6**。
> 真实帧成本 **1.30ms**（Apple M4 Max / 3200×1800 / 315 单位，解除 vsync 实测）≈ 60FPS 预算的 13 倍余量。
