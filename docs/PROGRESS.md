...
> 最后更新：2026-09-23（第三轮：战争迷雾 / 装备·Loot·天赋闭环 / 第 5 首音乐）
...
`npm test` **60/60**（dev）· `npm run test:prod` **60/60**（生产构建）· `npm run test:gpu` **8/8**（vsync + 解除上限两种模式）
· `npm run test:layout` **24/24**（6 视口 + 4 个菜单页）· `npm run test:soak` **6/6**（无泄漏）。

**第三轮做完的事**：战争迷雾（含 explore 目标）、装备掉落 + 装备界面 + 属性入局、天赋树（可加点/可退还）、第 5 首音乐轨。
真实帧成本仍是 **1.30ms**（迷雾只加了 0.08ms），60FPS 预算余量约 13 倍。
...
2. **地形美术偏「方块拼贴」**：草地用 3 色 40px 方块变化，远看有明显补丁感。
...
1. **战役 02–10 布点 + 两张地图**（现在最大的内容缺口）：用 `MapGen.generateMap()` 的 biome 参数为
   Dark Forest / Ruined Fortress 写布点函数（照抄 `buildGreenValley` 的结构），每关按 `MISSIONS` 里的
   camps/boss/objectives 跑通。判据：每关都能用 smoke 模板跑到 Victory。
2. **地形美术**：给 `TerrainPainter` 加一层低幅噪声/细节（或草簇贴图），消掉草地补丁感。
   判据：1600×900 视口截图里看不到规则方块边界。
3. **窄路口排队/让路**：宽 1 格通道下 20 单位通过不互相顶死。
   判据：全部通过且最差间距比 ≥0.8（复用 formation 测试的度量）。
4. **池化尸体/FX**（可选，已证实不泄漏）。
5. **手机端**：如果要做，先做菜单页重排（当前明确溢出），再做 HUD 竖屏堆叠。
