...
## 第十二轮：完整版包**上传完成**并备好交付文案

爱发电商品此前**已在售但无交付**（买家付 ¥7 拿不到东西）——这是比"链接不够私密"更严重的问题。
本轮补上：

| 步骤 | 结果 |
|---|---|
| 上传完整版包 | ✅ 试了 4 个托管站：catbox 拒绝（`Invalid uploader`）、0x0.st 已停止上传、**litterbox 成功** |
| 直链 | `https://litter.catbox.moe/2spge1.zip` · **641,094 字节**（与本地包一致）· 上传后 HTTP 200 实测 |
| 交付入口 | 后台「商品编辑页 → 高级选项 → **兑换链接**」→ `/setting/redeem?plan_id=…&sku_id=…`（可生成兑换码） |
| 自动回复字段 | ❌ 在 `/setting/account`、`/setting/data`、商品编辑页、电铺设置里**均未找到**（你 7 月给 AI 包设置时用的是 cua-driver+Safari；该功能疑在订单发货流程或被未认证状态限制） |
| 交付文案 | ✅ 已写入 `docs/AFDIAN-DELIVERY.md` 并**放进系统剪贴板**（含直链 + 试玩链接 + 失效补发说明） |
| ⚠️ 注意 | litterbox **只保 72 小时** → 正式发布请改传网盘（夸克），保留"失效请回复补发" |


---

## 第十三轮：GitHub Release 资产**换成试玩版**（原先的完整版包已过时且不该公开）

排查发现 Release 里的 `aetheria-dawnwake-html5.zip`（436KB）是 **demo/完整版分离之前**构建的旧包，
而且它是**完整版**——放在公开 Release 会削弱 itch $1 / 爱发电 ¥7 的付费层。

**决策依据（查了你的其他仓库）**：你 15 个游戏仓库**几乎全是 PUBLIC**（含 stormfront / neo-drift /
dark-zone / horizon-rush 这些付费游戏），但**它们一个都没有 Release 资产**——
即约定是「公开源码、不公开预构建包」。
...
| 动作 | 结果 |
|---|---|
| 删除旧资产 | ✅ `aetheria-dawnwake-html5.zip` 已删（它是旧代码的完整版包） |
| 上传新资产 | ✅ `aetheria-demo.zip`（**641,202 字节**，试玩版：包内可玩集 `["m01","m02"]` 已解包核对） |
| Release 说明重写 | ✅ 分「试玩（免费·前两关）」与「完整版（十关·$1/¥7）」两节，含三个在线链接与源码链接 |
