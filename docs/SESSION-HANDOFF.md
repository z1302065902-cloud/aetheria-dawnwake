...
> 保存点：`0ca8887`（"fix(economy): the hardcoded tile size..."）
...
### 下一步第一优先：让 m01 在 par time 内通关（pacing 4/5）

节奏已达标三项：**升级 3:00 · 部队成型 4:07 · 首场大会战 17:20**（设计窗口 ≤10:00 / ≤12:00 / 10:00–18:00）。
差的最后一项是"20 分钟通关"。

**最后一次实测的现象**：脚本玩家在 4:08 就带着 6-8 人部队开向**最近的**营地，
17:20 爆发大会战，但 `o4 摧毁营地` 始终 `active`，营地血量没被打掉。

**下一步要量的东西**（都已经有现成工具）：
```bash
npx vite --port 5173 --strictPort &
node tests/pacing.mjs            # 4/5，看 4:08 推进 / 17:20 会战
```
把它改成每 5 秒打印：`army.length`、`camp.hp`、`camp` 周围 260px 内的我方单位数、部队 state。
**要判断的是**：部队到底有没有走到营地、有没有在打建筑。
- 如果 `armyNearCamp` 一直是 0 → 是**移动/寻路**问题（部队走不到营地，可能是被守军挡住或路径终点吸附问题）
- 如果 `armyNearCamp > 0` 但 `camp.hp` 不掉 → 是**目标选择**问题（`Orders.acquire` 的建筑回退没生效）
- 如果部队在路上就被磨光 → 是**难度曲线**问题（再调营地守军/波次）

另外注意：m01 有 **2 座营地**（强度 2 和 3），主线只需要摧毁**一座**。

### 次要待办

1. 结算页展示本局祝福 + 冒险发现清单（"再玩一局"的动力）。
2. `m01` 的教程引导（5 分钟内理解玩法）目前靠 HUD 面板与横幅，没有分步提示。
3. 每座营地都带一座 `vb_altar`（连 m01 的荒野氏族营地也有），内容上不协调，可改为只给虚空族。
