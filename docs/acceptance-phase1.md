...
>
> 自动化命令：`node tests/smoke.mjs`（dev）与 `node tests/smoke.mjs http://localhost:4173`（生产构建）。
> 最近一次结果：**dev 32/32 通过；生产构建 32/32 通过**（2026-09-23）。
...
- 差距：**尸体未池化**；`FxSystem.death/explosion/telegraph` 每帧仍在 `add.image` + tween（属于短命对象，量大时有分配压力）。
...
### D-04 真实设备 60FPS / 多分辨率回归
- 标准：普通桌面 60FPS；1280×720 起可用。
- 验证方式：应在带真实 GPU 的浏览器里测 FPS，并用 4 设备矩阵（桌面/平板/手机横竖）跑布局回归。
- 状态：**☐ 未实现**
- 实测结果：沙盒是 headless + SwiftShader **软件光栅化**，帧时间 avg 25.4ms / worst 33.4ms 是被 rAF(~30Hz) 限制的结果，**不能作为 GPU 性能证据**。
- 下一步：`npm run dev` 后在 Chrome 里开 FPS 显示（HUD 右上角已内建 FPS）实测；再用 Playwright 4 设备矩阵补布局断言。
...
### E-04 Roguelite 遗物永久成长
- 状态：◐ 部分实现
- 实测结果：遗物作为可选目标奖励会发放并写入存档，英雄界面会显示已获得（✔）。
- 差距：**加成数值没有作用到战斗**（`RelicDef.effect` 未被任何系统读取）。
